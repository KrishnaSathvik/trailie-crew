...
## Phase 4B public-share boundary

`plan_share_links` has RLS enabled plus a restrictive deny policy and no direct anonymous, authenticated, or service-role table grants. Authenticated users call only narrow status/create/revoke RPCs; creation and revocation re-verify `auth.uid()`, participant ownership, active host role, room ownership, and exact PASS-published plan state. Rotation takes a transaction-scoped advisory lock, revokes the previous active same-version row, and preserves its audit row.

The raw 256-bit token exists only in trusted Node memory and the one-time response. PostgreSQL stores SHA-256 and a non-secret eight-character display prefix. Anonymous users cannot invoke verification. The server secret calls one hash verifier that checks active status, expiration, immutable plan hash, public snapshot hash, and exact version before returning only the snapshot. Security-definer functions use `search_path=''` and explicit grants. Realtime contains identifiers and event type only—never tokens, URLs, snapshots, visitors, or paths.

No visitor access events, IP hashes, public storage bucket, signed artifact URL, or persisted export rows are collected in Phase 4B. The only export operational record is a private, one-day rate event containing room, exact plan/version, export type, and authenticated user ID. It enforces 30 calendar or 10 print requests per user/room/type per ten minutes. ICS remains lightweight and authenticated on demand, and PDF uses an authenticated print route/browser Save as PDF.
