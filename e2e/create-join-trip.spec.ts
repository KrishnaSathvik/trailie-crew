...
test("two anonymous users create and join one RLS-protected Trip", async ({
  ...
  await hostPage.getByLabel(/Expected travelers/).fill("4");
  ...
  await hostPage.getByRole("button", { name: "Copy invitation URL" }).click();
  await expect(hostPage.getByText("Invitation URL copied.")).toBeVisible();

  const memberContext = await browser.newContext();
  const memberPage = await memberContext.newPage();
  const memberProblems = collectConsoleProblems(memberPage);
  await memberPage.goto(invitePath);
  await expect(memberPage.getByText("Invitation ready")).toBeVisible();
  ...
  await expect(hostPage.getByLabel("One-time invitation URL")).toHaveCount(0);
...
test("entry routes remain usable on mobile, by keyboard, and in both themes", async ({
  ...
  await page.goto("/");
  await expect(page.getByRole("link", { name: "Create a Trip" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Join a Trip" })).toBeVisible();
  expect(
    await page.evaluate(
  ...
  await expect(
    page.getByRole("heading", { name: "Name the adventure." }),
  ).toBeVisible();
  await expect(
