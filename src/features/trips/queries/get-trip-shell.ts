...
import { getInitialRoomMessages } from "@/features/chat/queries/get-room-messages";
...
export async function getTripShell(
  ...
  const [roomResult, participantsResult, inviteResult, initialMessages] =
    await Promise.all([
      client
        .from("rooms")
        .select(
      ...
      client
        ...
        .limit(1)
        .maybeSingle(),
      getInitialRoomMessages(client, roomId),
    ]);
  ...
  try {
    ...
    return {
      ...
      initialHistoryError: initialMessages === null,
    };
  } catch {
    return null;
  }
