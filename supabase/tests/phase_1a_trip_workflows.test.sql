...
select throws_ok(
  $$ select * from public.create_trip_unprotected('No identity', 'Nobody', null) $$,
  'P0001',
...
select * from public.create_trip_unprotected('Boundary Waters', 'Maya', 6);
reset role;
...
select * from public.create_trip_unprotected('Second Trip', 'Maya', null);
reset role;
...
select set_config('request.jwt.claim.sub', '10000000-0000-4000-8000-000000000001', true);
set local role authenticated;
select throws_ok($$ select * from public.create_trip_unprotected('', 'Maya', null) $$, 'P0001', 'Trip name must be between 1 and 100 characters.', 'blank Trip name fails');
select throws_ok($$ select * from public.create_trip_unprotected(repeat('x', 101), 'Maya', null) $$, 'P0001', 'Trip name must be between 1 and 100 characters.', 'long Trip name fails');
select throws_ok($$ select * from public.create_trip_unprotected('Trip', '', null) $$, 'P0001', 'Display name must be between 1 and 50 characters.', 'blank display name fails');
select throws_ok($$ select * from public.create_trip_unprotected('Trip', repeat('x', 51), null) $$, 'P0001', 'Display name must be between 1 and 50 characters.', 'long display name fails');
select throws_ok($$ select * from public.create_trip_unprotected('Trip', 'Maya', 0) $$, 'P0001', 'Expected travelers must be between 1 and 50.', 'zero expected travelers fails');
select throws_ok($$ select * from public.create_trip_unprotected('Trip', 'Maya', 51) $$, 'P0001', 'Expected travelers must be between 1 and 50.', 'too many expected travelers fails');
reset role;
...
select * from public.join_trip_unprotected((select invite_token from created_one), 'Leo');
reset role;
...
select * from public.join_trip_unprotected((select room_code from created_two), 'Nia');
select is((select room_id from joined_code), (select room_id from created_two), 'a valid short room code joins successfully');
select set_config('request.jwt.claim.sub', '10000000-0000-4000-8000-000000000002', true);
select throws_ok(
  format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from created_one), 'Another Name'),
...
set local role authenticated;
select throws_ok(
  format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from created_one), 'leo'),
...
select set_config('request.jwt.claim.sub', '10000000-0000-4000-8000-000000000001', true);
set local role authenticated;
create temporary table revoked_trip as select * from public.create_trip_unprotected('Revoked Trip', 'Maya', null);
create temporary table expired_trip as select * from public.create_trip_unprotected('Expired Trip', 'Maya', null);
create temporary table limited_trip as select * from public.create_trip_unprotected('Limited Trip', 'Maya', null);
create temporary table archived_trip as select * from public.create_trip_unprotected('Archived Trip', 'Maya', null);
create temporary table deleted_trip as select * from public.create_trip_unprotected('Deleted Trip', 'Maya', null);
reset role;
...
select set_config('request.jwt.claim.sub', '10000000-0000-4000-8000-000000000004', true);
set local role authenticated;
select throws_ok(format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from revoked_trip), 'Ari'), 'P0001', 'Invite is revoked.', 'revoked invite fails');
select throws_ok(format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from expired_trip), 'Ari'), 'P0001', 'Invite has expired.', 'expired invite fails');
create temporary table limited_join as select * from public.join_trip_unprotected((select invite_token from limited_trip), 'Ari');
select throws_ok(format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from archived_trip), 'Ari'), 'P0001', 'Trip is not active.', 'archived Trip fails');
select throws_ok(format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from deleted_trip), 'Ari'), 'P0001', 'Trip is not active.', 'deleted Trip fails');
reset role;

select set_config('request.jwt.claim.sub', '10000000-0000-4000-8000-000000000005', true);
set local role authenticated;
select throws_ok(format('select * from public.join_trip_unprotected(%L, %L)', (select invite_token from limited_trip), 'Bea'), 'P0001', 'Invite has reached its usage limit.', 'max uses is enforced');
reset role;
